class wkhtmltopdf{
	...
	exec { '/usr/bin/wget https://wkhtmltopdf.googlecode.com/files/wkhtmltopdf-0.11.0_rc1-static-amd64.tar.bz2':
		unless => ["/usr/bin/test -f /usr/local/bin/wkhtmltopdf"],
		       notify => Exec['/bin/tar -C /usr/local/bin -jxvf wkhtmltopdf-0.11.0_rc1-static-amd64.tar.bz2'],
	}
	exec{ '/bin/tar -C /usr/local/bin -jxvf wkhtmltopdf-0.11.0_rc1-static-amd64.tar.bz2':
		refreshonly => true,
